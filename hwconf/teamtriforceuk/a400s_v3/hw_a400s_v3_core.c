...
static void hw_a400s_set_hardware_current_limits(void);
...
void hw_init_gpio(void) {
	...
	hw_a400s_reset_faults(); // Handshake with hardware protection
}
...
void hw_a400s_reset_faults(void) {		
	...
	if(imu_startup_done()) {
		...
		if(hw_a400s_hardware_handshake()) {	
			uint8_t txb[1];	
			
			// Setup current limit digipots at startup they will default to 0A
			hw_a400s_set_hardware_current_limits();			
